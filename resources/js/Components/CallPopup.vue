...
watch(
  () => [props.localStream, props.isVideoCall, props.isCameraEnabled, props.isCallActive, props.isCallOutgoing],
  () => {
    nextTick(() => {
...
watch(
  () => [props.remoteStream, props.isCallActive],
  () => {
    nextTick(() => {
...
// Handle Speaker Mute
watch(() => props.isSpeakerMuted, (isMuted) => {
  if (remoteAudioRef.value) {
    remoteAudioRef.value.muted = isMuted
  }
...
// Prevent body scroll when call is active, outgoing, or incoming
watch(
  () => props.isCallActive || props.isCallIncoming || props.isCallOutgoing,
  (val) => {
    if (val) {
...
<template>
  <Teleport to="body">
    <!-- 1. Incoming Call Notification (المستقبل فقط) -->
    <div
      v-if="isCallIncoming"
    ...
    >
      <div class="bg-white rounded-3xl w-full max-w-sm p-8 text-center shadow-2xl animate-bounce-in">
        ...
        <h3 class="text-xl font-bold text-gray-800 mb-1">{{ isVideoCall ? '  vidieo call' : 'voice call' }}</h3>
        <p class="text-gray-500 mb-6">{{ callerName }} contact with you...</p>
    ...
    <!-- 2. Active Call & Outgoing Overlay (المتصل والمستقبل بعد القبول) -->
    <div
      v-if="isCallActive || isCallOutgoing"
    ...
    >
      ...
      <template v-if="isVideoCall">
        ...
        <!-- Fallback avatar if no remote video yet or call outgoing -->
        <div
          v-if="!remoteStream || isCallOutgoing"
          class="absolute inset-0 flex flex-col items-center justify-center bg-gray-900 z-0"
        >
          <div class="w-28 h-28 rounded-full bg-gradient-to-br from-green-400 to-green-600 text-white flex items-center justify-center mb-4 shadow-2xl animate-pulse">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-14 w-14" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" />
            </svg>
          </div>
          <h2 class="text-2xl font-bold text-white mb-1">
            {{ isCallOutgoing ? 'Calling...' : callerName || 'In Call' }}
          </h2>
          <p class="text-gray-400 text-lg font-mono">
            {{ isCallOutgoing ? 'Waiting for response...' : formatDuration(callDuration) }}
          </p>
        </div>

        <!-- Top Overlay: caller info -->
        <div class="absolute top-0 inset-x-0 p-6 bg-gradient-to-b from-black/60 to-transparent flex items-center justify-between z-10">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h2 class="text-lg font-bold text-white">{{ callerName || 'Unknown User' }}</h2>
              <p class="text-gray-300 text-sm font-mono">{{ isCallOutgoing ? 'calling...' : formatDuration(callDuration) }}</p>
            </div>
          </div>
          <span
            class="px-3 py-1 rounded-full bg-yellow-500/30 border border-yellow-400/50 backdrop-blur text-yellow-200 text-xs font-bold animate-pulse"
            v-if="isCallOutgoing"
          >
            جارٍ الاتصال...
          </span>
        ...
        <!-- Local Video PiP -->
        <div class="absolute top-4 left-4 w-32 h-48 sm:w-40 sm:h-56 rounded-2xl overflow-hidden shadow-2xl border-2 border-white/30 bg-black z-20">
          <video
        ...
        <!-- Bottom Controls -->
        <div class="absolute bottom-8 inset-x-0 flex justify-center z-20">
          <div class="flex items-center gap-6 bg-black/40 backdrop-blur px-8 py-4 rounded-full shadow-2xl">
            ...
            <button
              ...
              :title="isMicMuted ? 'cancel mute Mic' : 'mute Mic'"
            >
            ...
            <button
              ...
              :class="isSpeakerMuted
                ? 'bg-red-500 text-white hover:bg-red-600'
                : 'bg-white/15 text-white hover:bg-white/25'"
              :title="isSpeakerMuted ? 'تشغيل السماعة' : 'mute السماعة'"
            >
      ...
      <template v-else>
        ...
        <div class="text-center mb-12">
          <div class="w-24 h-24 mx-auto rounded-full bg-gradient-to-br from-green-400 to-green-600 text-white flex items-center justify-center mb-4 shadow-2xl animate-pulse">
